...
class MultiEngineExtractor:
    ...
    def __init__(self, working_dir="./soil_reports"):
        ...
        self.soil_parameters = {
            ...
            'Sulphur': {
                'unit': 'mg/kg',
                'patterns': [r'sulphur[\s:]*(\d+\.?\d*)', r'available\s+sulphur[\s:]*(\d+\.?\d*)'],
                'range': (5.0, 100.0),
                'value': None,
                'confidence': None,
                'strict': True
    ...
    def convert_pdf_to_images(self, pdf_path: str) -> List[str]:
        """Convert PDF pages to enhanced images for OCR"""
        try:
            ...
            for i, image in enumerate(images):
                ...
                path = os.path.join(self.working_dir, f'page_{i+1}.jpg')
    ...
    def extract_parameter_value(self, text: str, parameter: str, param_info: Dict) -> Optional[float]:
        ...
        if 'sulphur' in param_key or 'sulfur' in param_key:
            relaxed_variants += ['sulfur', 'sulphur (s)', 'available sulphur']
    ...
    def process_file(self, filepath: str) -> Dict:
        ...
        if ext == "pdf":
            ...
            detected_count = sum(1 for p in self.soil_parameters.values() if p['value'] is not None)

            if detected_count < 8 and convert_from_path:
                image_paths = self.convert_pdf_to_images(filepath)
                for img_path in image_paths:
                    ocr_text = self.extract_with_ocr(img_path)
                    text_collected += ocr_text + "\n"
                self.extract_from_text(text_collected)
...
SoilExtractor = MultiEngineExtractor
